...
esp_err_t init_gpio()
{
	...
	/*Dissolved Oxygen Sensor*/
	adc2_config_channel_atten(OXYGEN_CHANNEL, ADC_ATTEN_DB_2_5); //on adc2!!!
...
esp_err_t gather_data(datapoint_t* dp)
{
	...
	for (int i = 0; i < NO_OF_SAMPLES; i++) 
	{
		...
		/*OXYGEN TESTING*/
		//oxygen1_raw = adc2_get_raw((adc2_channel_t)OXYGEN_CHANNEL);
		
		// Declare an integer to hold the raw ADC reading
		int raw_out;
		...
		// Call adc2_get_raw to get a raw ADC reading. The arguments are:
		// - The ADC channel connected to the O2 sensor
		// - The bit width of the ADC
		// - A pointer to an integer to store the raw ADC reading
		esp_err_t ret = adc2_get_raw((adc2_channel_t)OXYGEN_CHANNEL, ADC_WIDTH_BIT_12, &raw_out);

		// Check the return value of adc2_get_raw to determine if the reading was successful
		if (ret == ESP_OK) {
			// If the reading was successful, assign the raw ADC reading to oxygen1_raw
			oxygen1_raw = raw_out;
		} else {
			// If the reading was not successful, print an error message
			printf("Failed to read ADC\n");
		}
		oxygen1 += get_raw_data_oxygen(chars_2_5_dB, oxygen1_raw);
	...
	/*TURB2 TESTING*/
	esp_logi_result(temp1, temp2, temp3, turbd1, turbd2, light1, salin1, salin2, oxygen1);
...
Return:
	N/A
*/
void esp_logi_result(float temp1, float temp2, float temp3, float turbd1, float turbd2, float light1, float salin1, float salin2, float oxygen1)
{
	...
	/*O2 Sensor*/
	ESP_LOGI(GPIO_TAG, "Dissolved Oxygen: %f", oxygen1);
}