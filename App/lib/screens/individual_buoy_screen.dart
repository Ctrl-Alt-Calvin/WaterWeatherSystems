...
class _IndividualBuoyState extends State<IndividualBuoyScreen> {
  ...
  void collectRecordedData() async {
    ...
    for (var i = 0; i < val[0]; i++) {  // from i = 0 to length of data instances
      ...
      // print('RAW DATA FROM THE BUOY ${rawData}');
      print('===== READ $i =====');
      print('time:        ${rawData[dataOffset++] | (rawData[dataOffset++] << 8) | (rawData[dataOffset++] << 16) | (rawData[dataOffset++] << 24)}'); // SECONDS
      print('temp1:       ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('temp2:       ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('temp3:       ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('salinity:    ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('salinity2:   ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('light:       ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('turbidity1:  ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('turbidity2:  ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      print('oxygen:      ${rawData[dataOffset++] | (rawData[dataOffset++] << 8)}');
      ...
      dataPoints.add(
          BuoyData(
              rawData[dataOffset++] | (rawData[dataOffset++] << 8) | (rawData[dataOffset++] << 16) | (rawData[dataOffset++] << 24), // SECONDS
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // CELSIUS
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // CELSIUS
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // CELSIUS
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // SALINITY1
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // SALINITY2
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // LIGHT
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // TURBIDITY1
              rawData[dataOffset++] | (rawData[dataOffset++] << 8), // TURBIDITY2
              rawData[dataOffset++] | (rawData[dataOffset++] << 8)  // OXYGEN
          )
      );
  ...
  Widget build(BuildContext context) {
    ...
    var dataPoints = [
      BuoyData(300, 20, 26, 32, 25, 31, 110, 45, 38, 12),
      BuoyData(600, 19, 24, 31, 24, 30, 105, 47, 40, 12),
      BuoyData(900, 21, 25, 29, 26, 32, 102, 43, 36, 12),
      BuoyData(1200, 18, 23, 30, 23, 29, 100, 46, 39, 12),
      BuoyData(1500, 22, 27, 33, 27, 33, 108, 44, 37, 12),
      BuoyData(1800, 19, 26, 32, 25, 31, 109, 42, 35, 12),
      BuoyData(2100, 20, 24, 31, 24, 30, 107, 48, 41, 12),
      BuoyData(2400, 21, 25, 30, 26, 32, 101, 45, 38, 12),
      BuoyData(2700, 18, 23, 30, 23, 29, 106, 47, 40, 12),
      BuoyData(3000, 22, 27, 33, 27, 33, 103, 43, 36, 12),
      BuoyData(3300, 19, 26, 32, 25, 31, 104, 46, 39, 12),
      BuoyData(3600, 20, 24, 31, 24, 30, 111, 44, 37, 12),
      BuoyData(3900, 21, 25, 29, 26, 32, 107, 42, 35, 12),
      BuoyData(4200, 18, 23, 30, 23, 29, 102, 48, 41, 12),
      BuoyData(4500, 22, 27, 33, 27, 33, 105, 45, 38, 12),
    ];
                            ...
                            child: Padding(
                              ...
                              child: Container(
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    if (buoyIDs.authLevel == 'owner' || buoyIDs.authLevel == 'manager' || true) {
                                      handleLocation();
                                    }
                                    else {
